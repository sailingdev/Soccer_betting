...
def get_data_from_hometeam(team_info, team_id, season_id):
	...
	if len(playersdata):
		return_val = {"home_team_players":playersdata,"home_total_score":team_score, "home_team_strength":get_strength(team_score), "dynamic_home_team_ranking":get_last_dynamic_ranking(team_id, True), "static_home_team_ranking": get_last_static_home_ranking(team_id, season_id)}
	else:
		print("none")
		return_val = {}
	return return_val
...
def get_data_from_awayteam(team_info, team_id, season_id):
	...
	if tr_results:
		for i in range(0, 11):
			...
			playersdata.append(data)
	if playersdata:
		return_val = {"away_team_players":playersdata, "away_total_score":team_score, "away_team_strength":get_strength(team_score), "dynamic_away_team_rankig":get_last_dynamic_ranking(team_id, False),  "static_away_team_ranking": get_last_static_away_ranking(team_id, season_id)}
	else:
		print("none")
		return_val = {}
	return return_val
...
def get_last_static_home_ranking(team_id , season_id):
	sql = f'SELECT S_H_ranking FROM season_league_team_info WHERE team_id = {team_id} AND season_id = {season_id}'
	mycursor.execute(sql)
	result = mycursor.fetchall()
	if result: 
		ranking = result[0][0]
...
def get_last_static_away_ranking(team_id , season_id):
	sql = f'SELECT S_A_ranking FROM season_league_team_info WHERE team_id = {team_id} AND season_id = {season_id}'
	mycursor.execute(sql)
	result = mycursor.fetchall()
	if result: 
		ranking = result[0][0]
...
def main():
	if len(sys.argv) > 6:
		...
		text = doing_team_news(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6]);
